...
def produceObj(filename, sessionToken):
    compileCommand = f'g++ -g -std=c++11 -I. {os.path.join(workdir, sessionToken, filename)} -o {os.path.join(workdir, sessionToken, outObjName)}'
    os.system(compileCommand)

def processFile(path, filename, sessionToken):
    print('Filename is ', filename)
    os.system(f'mkdir {os.path.join(workdir, sessionToken)}')
    os.system(f'mv {os.path.join(path, filename)} {os.path.join(workdir, sessionToken, filename)}')
...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            response = gdbmis[sessionToken].write(f'-file-exec-and-symbols {os.path.join(workdir, sessionToken, outObjName)}')
            response.extend(gdbmis[sessionToken].write(f'skip -gfi /usr/include/c++/7/bits/*.h'))
