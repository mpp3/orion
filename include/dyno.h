...
void Memory::free(void *address)
{
	...
	for (int i{0}; i < allocations_; ++i)
	{
		if (addresses_[i] == address)
		{
			for (int j{i}; j < allocations_ - 1; ++j) {
				addresses_[j] = addresses_[j + 1];
				sizes_[j] = sizes_[j + 1];
			}
			addresses_[allocations_ - 1] = nullptr;
			sizes_[allocations_ - 1] = 0;
			allocations_--;
		}
	}
}
...
class OutputRedirector {
	public:
	OutputRedirector(std::string output_filename)
	{
		strm = freopen(output_filename.c_str(), "w", stdout);
		if (strm) {
			setvbuf(strm, nullptr, _IONBF, 0);
		}
	}
	private:
	FILE* strm;
...
OutputRedirector oredir{"output.txt"};
